...
DEFINITION OF A "CHARGING EVENT":
  A charging event is a contiguous run of hours within a day where the SoC
  rises by at least MIN_CHARGE_RISE percentage points in total (not just a
  brief 1-2% blip). The "charging time" representing that event is taken as
  the MIDPOINT hour of the contiguous rising run — this is the most
  defensible single timestamp to call "when charging happened" for a
  multi-hour charging session (e.g. plugged in at 11pm, still charging at
  2am -> charging time ~ 00:30, rounded to hour 0 or 1).

  Both ACTUAL and PREDICTED daily dSocdt/Soc curves are scanned with the same
  event-detection logic, so the comparison is apples-to-apples.

METRIC:
  For each day, actual charging events are matched to predicted charging
  events (nearest-hour matching). The absolute hour difference between each
  matched pair is the "error" for that event. MAE is the mean of all these
  per-event errors across all devices and days.

  Additional interpretive metrics:
    - Detection rate    : % of days with an actual charging event where the
                           model also predicted at least one event that day
    - False alarm rate   : % of days with NO actual charging event where the
                           model predicted one anyway
    - Event count match   : how often the model predicts the same NUMBER of
                           charging events as actually occurred
...
Outputs (all inside results/xgboost/charging_schedule/):
  ...
  charging_schedule_summary.csv               — per-device MAE, detection rate, etc.
  ...
  charging_schedule_detection_plot.png        — detection rate / false alarm rate per device
  charging_schedule_sample_days.png           — sample days showing actual vs predicted
...
MIN_CHARGE_RISE_ACTUAL = 0.40

MIN_HOURLY_RISE_PRED = 0.004
MIN_CHARGE_RISE_PRED = 0.25
...
def detect_charging_events(dsocdt_24h, min_hourly_rise, min_total_rise):
    ...
    SUM of dSocdt across the run (total SoC gained) >= min_total_rise.

    The representative hour for an event is the midpoint of the run,
    weighted toward the hour with the largest single-hour rise (so a run
    like [+1, +8, +1] is centered near the +8 hour, not a flat midpoint).
    ...
    while i < n:
        ...
        if not np.isnan(val) and val > min_hourly_rise:
            ...
            while j < n and not np.isnan(dsocdt_24h[j]) and dsocdt_24h[j] > min_hourly_rise:
                ...
                j += 1
            total_rise = float(np.sum(run_vals)) * 60   # convert rate-sum (per-minute) to actual % SoC gained
            if total_rise >= min_total_rise:
                # Weighted-average hour, weighted by rise magnitude at each hour
...
def match_events(actual_events, predicted_events):
    ...
    plus counts of unmatched actual (missed) and unmatched predicted (false alarm).
    """
    ...
    n_missed       = len(actual_remaining)   # actual events with no predicted match
    n_false_alarm  = len(pred_remaining)     # predicted events with no actual match
    return matches, n_missed, n_false_alarm
...
def run_charging_schedule_predictions(wide, device_name):
    ...
    for predict_day_idx in range(TRAIN_DAYS, n_days):
        ...
        matches, n_missed, n_false_alarm = match_events(actual_events, predicted_events)
...
def plot_detection_rates(summary_df, plot_path):
    dev_data = summary_df[summary_df["device"] != "OVERALL_MEAN"]
    short_names = [d[:12] + "…" if len(d) > 12 else d for d in dev_data["device"]]
    ...
    ax.bar(x - width/2, dev_data["detection_rate_%"], width,
           label="Detection rate (%)", color="seagreen", edgecolor="white")
    ax.bar(x + width/2, dev_data["false_alarm_rate_%"], width,
           label="False alarm rate (%)", color="salmon", edgecolor="white")
    ax.set_xticks(x)
    ax.set_xticklabels(short_names, rotation=30, ha="right", fontsize=8)
    ax.set_ylabel("Rate (%)", fontsize=11)
    ax.set_ylim(0, 105)
    ax.set_title("Charging Event Detection Rate & False Alarm Rate per Device",
                 fontsize=12, fontweight="bold")
    ax.legend(fontsize=9)
    ax.grid(axis="y", linestyle="--", alpha=0.4)
    fig.tight_layout()
...
if __name__ == "__main__":
    ...
    for wf in wide_files:
        ...
        try:
            ...
            pred_rows = []
            all_device_errors = []
            n_days_with_actual_event   = 0
            n_days_detected            = 0
            n_days_no_actual_event     = 0
            n_days_false_alarm         = 0
            ...
            for r in results:
                ...
                has_actual = len(r["actual_events"]) > 0
                has_pred   = len(r["predicted_events"]) > 0

                if has_actual:
                    n_days_with_actual_event += 1
                    if has_pred and len(r["matches"]) > 0:
                        n_days_detected += 1
                else:
                    n_days_no_actual_event += 1
                    if has_pred:
                        n_days_false_alarm += 1
                ...
                pred_rows.append({
                    ...
                    "n_matched"         : len(r["matches"]),
                    "n_missed"          : r["n_missed"],
                    "n_false_alarm"     : r["n_false_alarm"],
            ...
            detection_rate = (100 * n_days_detected / n_days_with_actual_event
                              if n_days_with_actual_event > 0 else np.nan)
            false_alarm_rate = (100 * n_days_false_alarm / n_days_no_actual_event
                                if n_days_no_actual_event > 0 else np.nan)
            ...
            print(f"  Detection rate           : {detection_rate:.1f}%" if not np.isnan(detection_rate) else "  Detection rate: N/A")
            print(f"  False alarm rate         : {false_alarm_rate:.1f}%" if not np.isnan(false_alarm_rate) else "  False alarm rate: N/A")
            print(f"  Event count match rate   : {event_count_match_rate:.1f}%")

            all_summary_rows.append({
                "device"               : device_name,
                "total_days"           : len(results),
                "days_with_actual_event": n_days_with_actual_event,
                "mae_hours"            : round(device_mae, 4) if not np.isnan(device_mae) else np.nan,
                "detection_rate_%"     : round(detection_rate, 2) if not np.isnan(detection_rate) else np.nan,
                "false_alarm_rate_%"   : round(false_alarm_rate, 2) if not np.isnan(false_alarm_rate) else np.nan,
    ...
    if not summary_df.empty:
        ...
        overall_det = round(float(np.nanmean(summary_df["detection_rate_%"])), 2)
        overall_fa  = round(float(np.nanmean(summary_df["false_alarm_rate_%"])), 2)
        ...
        summary_df = pd.concat([
            ...
            pd.DataFrame([{
                ...
                "days_with_actual_event": summary_df["days_with_actual_event"].sum(),
                "mae_hours"             : overall_mae,
                "detection_rate_%"      : overall_det,
                "false_alarm_rate_%"    : overall_fa,
    ...
    if not summary_df.empty:
        ...
        plot_detection_rates(summary_df,
            os.path.join(OUTPUT_FOLDER, "charging_schedule_detection_plot.png"))
    ...
    for _, row in summary_df.iterrows():
        ...
        det_str = f"{row['detection_rate_%']:.1f}%" if not pd.isna(row['detection_rate_%']) else "N/A"
        fa_str  = f"{row['false_alarm_rate_%']:.1f}%" if not pd.isna(row['false_alarm_rate_%']) else "N/A"
        print(f"  {row['device']:<35}  MAE={mae_str:>10}  Detection={det_str:>7}  "
              f"FalseAlarm={fa_str:>7}  EventCountMatch={row['event_count_match_%']:.1f}%")
    print("=" * 65)
