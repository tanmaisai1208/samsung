...
Folder convention (as used by the project's git branches):
    results_1/   <- results copied from the "main" branch  (Approach 1)
    results_2/   <- results already in the "test" branch    (Approach 2)
...
# ─────────────────────────────────────────────────────────────
ROOT       = "."
R1         = os.path.join(ROOT, "results_1")   # Approach 1 (main branch)
R2         = os.path.join(ROOT, "results_2")   # Approach 2 (test branch)
OUTPUT_PDF = os.path.join(R2, "project_report.pdf")
...
def cover_page():
    ...
    elems.append(body(
        ...
        "(results_1/, main branch) clipped positive dSocdt values to zero and "
        ...
        "<b>Approach 2</b> (results_2/, test branch) retains positive dSocdt "
...
def toc():
    ...
    sections = [
        ...
        ("14", "Revised Statistical Analysis"),
        ("15", "Revised Hyperparameter Tuning"),
        ("16", "Revised Final Validation Results"),
        ("17", "Revised Feature Importance Analysis"),
        ("18", "Revised Tree Rule Extraction & Interpretation"),
        ("19", "Comparison: Approach 1 vs Approach 2"),
        ("20", "Next Steps"),
    ]
    for num, title in sections:
        if num in ("PART A", "PART B"):
            elems.append(sp(0.15))
            elems.append(Paragraph(f'<font name="Helvetica-Bold" color="#7a1f1f">{title}</font>', S["toc"]))
            elems.append(sp(0.1))
        else:
            elems.append(Paragraph(f'<font name="Helvetica-Bold">{num}</font>  {title}', S["toc"]))
    elems.append(PageBreak())
    return elems
...
def part_a_divider():
    elems = []
    elems.append(sp(6))
    elems.append(Paragraph("PART A", S["title"]))
...
def s3_preprocessing_v1():
    ...
    elems += section_header(
        "3.  Data Preprocessing (Interpolation-Based Resampling)", "h1")
    elems.append(body(
...
def s4_statistical_v1():
    elems = []
    elems += section_header(
        "4.  Statistical Analysis — Autocorrelation & Hurst Exponent", "h1")
    elems.append(body(
...
def s5_model_v1():
    elems = []
    elems += section_header("5.  XGBoost Model & Hyperparameter Tuning", "h1")
    elems.append(body(
...
def s6_final_validation_v1():
    elems = []
    elems += section_header("6.  Final Validation Results", "h1")
    elems.append(body(
...
def s7_feature_importance_v1():
    elems = []
    elems += section_header("7.  Feature Importance Analysis", "h1")
    elems.append(body(
...
def s8_tree_rules_v1():
    elems = []
    elems += section_header("8.  Tree Rule Extraction & Interpretation", "h1")
    elems.append(body(
    ...
    elems.append(body(
        "Consistent with feature importance, tree splits were dominated by "
        "dSocdt from the most recent day (day-minus-1), with evening hours "
        "(roughly 14:00–23:00) featuring most frequently as the decisive "
        "root-node condition."
    ))
    elems.append(PageBreak())
    return elems
...
def s9_model_complexity_v1():
    elems = []
    elems += section_header("9.  Model Size & Inference Time Analysis", "h1")
    elems.append(body(
...
def s10_charging_diagnostic():
    elems = []
    elems += section_header(
        "10.  Charging-Detection Diagnostic (Task A & Task B)", "h1")
    elems.append(body(
    ...
    elems.append(notebox(
        "Note: the Task A/B diagnostic plots below (charging_discrimination_plot.png, "
        "charging_event_detection_plot.png, charging_zero_threshold_sweep_plot.png) "
        "were generated on a different device/run and have not yet been copied "
        "into results_1/xgboost/final_validation/. Placeholders are shown until "
        "those files are added at the paths configured at the top of this script."
    ))
...
def s11_limitations():
    elems = []
    elems += section_header("11.  Limitations Identified in Approach 1", "h1")
    elems.append(warnbox(
...
def part_b_divider():
    elems = []
    elems.append(sp(6))
    elems.append(Paragraph("PART B", S["title"]))
...
def s12_motivation():
    elems = []
    elems += section_header("12.  Motivation for Methodology Revision", "h1")
    elems.append(bullet(
...
def s13_preprocessing_v2():
    elems = []
    elems += section_header(
        "13.  Revised Data Preprocessing (Average-Based Resampling)", "h1")
    elems.append(body(
...
def s14_statistical_v2():
    elems = []
    elems += section_header("14.  Revised Statistical Analysis", "h1")
    elems.append(body(
        "The Hurst exponent analysis was re-run across all device files "
        "available under Approach 2's revised resampling."
    ))
    hb = hurst_breakdown(PATHS_2["all_users_summary"])
    if hb:
        n_total, n_pers, n_rand, n_anti, mean_h = hb
        elems.append(results_table(
            ["Regime", "Device Count", "% of Devices"],
        ...
        elems.append(sp(0.3))
        elems.append(body(
            f"Mean Hurst exponent across all {n_total} devices: <b>{mean_h:.3f}</b>. "
            f"The persistence finding is reconfirmed under the revised resampling, "
            f"supporting continued use of a multi-day sequence-aware feature window."
        ))
    else:
        elems.append(body("[all_users_summary.csv not found for Approach 2 — "
                          "re-run battery_analysis-equivalent step if this analysis "
                          "was not repeated on the revised data.]"))
    elems.append(PageBreak())
    return elems

def s15_hyperparam_v2():
    elems = []
    elems += section_header("15.  Revised Hyperparameter Tuning", "h1")
    elems.append(body(
    ...
    param_order = [
        ("15.1", "n_estimators",  "10–250",           "200",  "val_curve_n_estimators.png"),
        ("15.2", "max_depth",     "2, 3, 4",          "2",    "val_curve_max_depth.png"),
        ("15.3", "learning_rate", "0.01–0.2",         "0.1",  "val_curve_learning_rate.png"),
        ("15.4", "subsample",     "0.7–0.95",         "0.95", "val_curve_subsample.png"),
    ]
...
def s16_final_validation_v2():
    elems = []
    elems += section_header("16.  Revised Final Validation Results", "h1")
    elems.append(body(
...
def s17_feature_importance_v2():
    elems = []
    elems += section_header("17.  Revised Feature Importance Analysis", "h1")
...
def s18_tree_rules_v2():
    elems = []
    elems += section_header(
        "18.  Revised Tree Rule Extraction & Interpretation", "h1")
    elems.append(body(
...
def s19_comparison():
    elems = []
    elems += section_header("19.  Comparison: Approach 1 vs Approach 2", "h1")
    elems.append(results_table(
...
def s20_next_steps():
    elems = []
    elems += section_header("20.  Next Steps", "h1")
    for item in [
        "Add the Task A/B charging-diagnostic plots to results_1/xgboost/final_validation/ "
        "so Section 10 renders with the actual figures instead of placeholders.",
        "SHAP analysis on the Approach 2 model — per-prediction feature attribution.",
        "Re-run model size / inference time analysis (Section 9) under Approach 2 "
        "for a direct complexity comparison alongside Section 19.",
        "LSTM / Transformer comparison under Approach 2's revised feature pipeline.",
        "Cross-user generalisation — train on pooled data, test on held-out devices.",
        "Final report write-up — formal academic-style documentation.",
    ]:
...
def build_pdf():
    ...
    story += s14_statistical_v2()
    story += s15_hyperparam_v2()
    story += s16_final_validation_v2()
    story += s17_feature_importance_v2()
    story += s18_tree_rules_v2()
    story += s19_comparison()
    story += s20_next_steps()
