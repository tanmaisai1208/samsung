...
Outputs:
  ...
  feature_importance_by_type.png  — grouped bar: dSocdt vs Soc vs Date features
  feature_importance_by_day.png   — how importance decays over the 14-day lookback
"""
...
# ─────────────────────────────────────────────────────────────
# BEST HYPERPARAMETERS  ← fill in your tuned values
# ─────────────────────────────────────────────────────────────
BEST_PARAMS = {
    "n_estimators"   : 25,
    "max_depth"      : 2,
...
# ─────────────────────────────────────────────────────────────
# FEATURE NAMES
# Bug fix: your original code had the day label reversed.
# day_label="day14" should be the oldest day (14 days back),
# day_label="day1"  should be the most recent day (1 day back).
# The loop goes train_start → train_end (oldest → newest),
# so offset = TRAIN_DAYS - day_position gives correct label.
# ─────────────────────────────────────────────────────────────
feature_names = []
for day_pos in range(TRAIN_DAYS):           # 0 = oldest, 13 = most recent
    days_back = TRAIN_DAYS - day_pos        # 14 = oldest, 1 = most recent
    day_label = f"day_minus{days_back}"     # e.g. day_minus14 … day_minus1
...
assert len(feature_names) == TRAIN_DAYS * FEATS_PER_DAY, \
    f"Feature name count mismatch: {len(feature_names)} vs {TRAIN_DAYS * FEATS_PER_DAY}"
...
# ─────────────────────────────────────────────────────────────
# Bug fix: original code clipped dSocdt to <=0 inside the loop
# but didn't do it here. Add the clip so feature importance is
# computed on the same data distribution the prediction model sees.
# ─────────────────────────────────────────────────────────────
all_X, all_y = [], []
...
for wf in wide_files:
    ...
    # Enforce discharge-only (same as in the main training pipeline)
...
# ─────────────────────────────────────────────────────────────
# Each estimator in MultiOutputRegressor is one XGBRegressor (one per hour).
# We average importance across all 24 hour-estimators.
importance_matrix = np.array([est.feature_importances_
                               for est in model.estimators_])   # (24, 728)
avg_importance    = importance_matrix.mean(axis=0)              # (728,)
...
# Tag each feature with its type and which day-back it belongs to
importance_df["feature_type"] = importance_df["feature"].apply(
    lambda f: "dSocdt" if f.startswith("dSocdt")
              else ("Soc" if f.startswith("Soc") else "Date")
)
...
colors = top30["feature_type"].map(
    {"dSocdt": "steelblue", "Soc": "darkorange", "Date": "seagreen"}
)

fig, ax = plt.subplots(figsize=(12, 10))
bars = ax.barh(range(len(top30)), top30["importance"].values,
...
ax.set_title("Top 30 Most Important Features\n"
             "(XGBoost with best hyperparameters — pooled across all devices)",
...
# ─────────────────────────────────────────────────────────────
# PLOT 2 — Importance by feature type (grouped)
# ─────────────────────────────────────────────────────────────
...
bar_colors = [{"dSocdt": "steelblue", "Soc": "darkorange",
               "Date": "seagreen"}[t] for t in type_summary["feature_type"]]
...
ax.set_title("Feature Importance by Type\n"
             "(Which category of input matters most overall)",
             fontsize=12, fontweight="bold")
...
# ─────────────────────────────────────────────────────────────
# PLOT 3 — Importance by days-back (how recency matters)
# ─────────────────────────────────────────────────────────────
...
width  = 0.28
types  = [c for c in ["dSocdt", "Soc", "Date"] if c in pivot.columns]
...
ax.set_title("Feature Importance by Recency\n"
